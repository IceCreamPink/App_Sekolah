<?php
    ...
    if(isset($_POST['update'])){
        extract($_POST);
        $update = mysqli_query($conn,"update tbsiswa set nis='$nis', nisn='$nisn',nama_siswa='$nama_siswa', alamat='$alamat', hp='$hp', agama='$agama', jk='$jk' where idsiswa=$id");
        var_dump($update);
        if($update){
            ?>
<script>
...
<?php
    ...
    if($id == ""){
        ...
        $query = mysqli_query($conn,"select * from tbsiswa where idsiswa=$id");
...
<body>
    ...
    <form action="?hal=edit" method="post">
        <table>
            <input type="hidden" name="id" value="<?=$baris['idsiswa']?>">
            <tr>
                <td>NIS</td>
                <td><input type="text" name="nis" placeholder="Nomor Induk Siswa" maxlength="20"
                        value="<?=$baris['nis']?>"></td>
            </tr>
            <tr>
                <td>NISN</td>
                <td><input type="text" name="nisn" placeholder="Nomor Induk Siswa Nasional" maxlength="10"
                        value="<?=$baris['nisn']?>"></td>
            </tr>
            <tr>
                <td>Nama Siswa</td>
                <td><input type="text" name="nama_siswa" placeholder="Nama Siswa" maxlength="50"
                        value="<?=$baris['nama_siswa']?>"></td>
            </tr>
            <tr>
                <td>Alamat</td>
