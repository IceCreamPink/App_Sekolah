    ...
    if(isset($_POST['simpan'])){
        ...
        if($password1 != "" && $password2 != ""){
          if($password1 == $password2){
            ...
            $up = mysqli_query($conn, "update tbuser set nama_user='$nama', username='$username', password='$pass' where iduser=$id");
          }else{
            echo "<script>alert('Update gagal, password tidak sama');location.href='?hal=tampiluser';</script>";
          }
        }else{
            $up = mysqli_query($conn, "Update tbuser set nama_user='$nama', username='$username' where iduser=$id");
        }
        if($up){
